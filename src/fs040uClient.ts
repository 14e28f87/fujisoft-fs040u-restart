...
interface RequestOptions {
  ...
  body?: string;
  /** 応答を待たない(端末が再起動して切断する)ことを許容するリクエスト用 */
  timeoutMs?: number;
}
...
async function postQuery(path: string, referer: string, cookie?: string): Promise<string> {
  return rawRequest({
    method: 'POST',
    path,
    headers: {
...
async function getText(path: string, referer: string, cookie: string): Promise<string> {
  return rawRequest({
    method: 'GET',
    path,
    headers: {
...
export async function login(): Promise<Fs040uSession> {
...
export async function triggerReboot(session: Fs040uSession): Promise<void> {
  try {
    await postQuery(
      ...
      `${BASE_URL}/start.html?id=reboot&reason=router_restart`,
      session.cookie,
    );
  } catch {
...
export async function waitForDeviceOnline(timeoutMs: number): Promise<boolean> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    try {
      await rawRequest({ method: 'GET', path: '/', headers: { 'User-Agent': USER_AGENT }, timeoutMs: 3_000 });
      return true;
    } catch {
